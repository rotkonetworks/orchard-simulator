// Web Worker that runs the real-Orchard WASM simulator off the main
// thread, so the browser stays interactive while keygen (~10s) and
// prove+verify (~30s) execute.
//
// Protocol:
//   main → worker: { type: 'init' }            → { type: 'inited' }
//   main → worker: { type: 'keygen' }          → { type: 'keygen-progress', phase, t0 }
//                                              → { type: 'keygen-done', elapsedMs }
//   main → worker: { type: 'prove', seed }     → { type: 'prove-progress', phase, t0 }
//                                              → { type: 'prove-done', demo, elapsedMs }
...
self.onmessage = async (e) => {
  ...
  try {
    ...
    if (type === 'keygen') {
      await ensureInit();
      self.postMessage({ type: 'keygen-progress', phase: 'building Orchard ProvingKey + VerifyingKey', t0: performance.now() });
    ...
    if (type === 'prove') {
      ...
      // Stage 1+2: sample witness + assemble Circuit.
      self.postMessage({
      ...
      // Stage 3: Proof::create → halo2_proofs::plonk::create_proof.
      self.postMessage({
        type: 'stage-start', stages: [3],
        label: 'create_proof (halo2_proofs::plonk::create_proof)',
      });
      ...
      // Stage 4: Proof::verify → halo2_proofs::plonk::verify_proof.
      self.postMessage({
        type: 'stage-start', stages: [4],
        label: 'verify_proof (halo2_proofs::plonk::verify_proof)',
      });
    ...
    if (type === 'two-proofs') {
      ...
      self.postMessage({
        type: 'stage-start', stages: [3],
        label: 'two independent create_proof calls on the same witness',
      });
